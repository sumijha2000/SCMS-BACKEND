...
namespace COMMON_PROJECT_STRUCTURE_API.services
{
    public class politics
    {
        ...
        public async Task<responseData> Politics(requestData rData)
        {
            responseData resData = new responseData();
            try
            {
                  var query = @"INSERT INTO mydb.politics_data(categories, description, video, video_name) values(@categories, @description, @video, @video_name)";
         ...
         public async Task<responseData> EditPolitics(requestData rData)
        {
            responseData resData = new responseData();
            try
            {
                var query = @"UPDATE mydb.politics_data SET categories=@categories, description=@description, video=@video, video_name=@video_name WHERE idpolitics_data=@id";
                MySqlParameter[] myParam = new MySqlParameter[]
                {
                    new MySqlParameter("@categories", rData.addInfo["categories"]),
                    new MySqlParameter("@description", rData.addInfo["description"]),
        ...
        public async Task<responseData> DeletePolitics(requestData rData)
        {
            responseData resData = new responseData();
            try
            {
                var query = @"DELETE FROM mydb.politics_data WHERE idpolitics_data=@id";
                MySqlParameter[] myParam = new MySqlParameter[]
                {
                    new MySqlParameter("@id", rData.addInfo["id"]),
                };

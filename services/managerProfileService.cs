...
namespace COMMON_PROJECT_STRUCTURE_API.services
{
    public class managerProfileService
    {
        ...
        public async Task<responseData> CreateManagerProfile(requestData rData)
        {
            responseData resData = new responseData();
            try
            {
                var query = @"INSERT INTO mydb.manager_profiles(name, email, phone, company_name, address, image) 
        ...
        public async Task<responseData> UpdateManagerProfile(requestData rData)
        {
            responseData resData = new responseData();
            try
            {
                var query = @"
                    UPDATE mydb.manager_profiles 
        ...
        public async Task<responseData> DeleteManagerProfile(requestData rData)
        {
            responseData resData = new responseData();
            try
            {
                var query = @"
                    DELETE FROM mydb.manager_profiles 
                    WHERE email = @email;
                ";
        ...
        public async Task<responseData> GetManagerProfileDatal(requestData rData)
{
    responseData resData = new responseData();
    try
    {
        var query = @"SELECT name, email, phone, company_name, address, image FROM mydb.manager_profiles WHERE email=@email";
        MySqlParameter[] myParam = new MySqlParameter[]
        {
            new MySqlParameter("@email", rData.addInfo["email"])
        };
